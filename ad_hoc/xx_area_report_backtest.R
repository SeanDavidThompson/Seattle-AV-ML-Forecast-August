...
std_actuals <- function(raw) {
  ...
  col[["spec_region"]] <- pick_col(nms, NULL, c("^spec_region\\z"))
  ...
  act <- data.table(
    ...
    spec_region = norm_key(col_chr(raw, col[["spec_region"]]))
  )
  ...
  message(sprintf("  Actuals kept: %d of %d rows  (res areas %d | com areas %d | specialty %d, of which regions %d | condo areas %d)",
                  nrow(act), n0,
                  act[track == "res" & kind == "geo", .N],
                  act[track == "com" & kind == "geo", .N],
                  act[kind == "specialty", .N],
                  act[level == "spec_region", .N],
                  act[kind == "condo", .N]))
...
# Long map of (specialty, neighborhood) -> region key
spec_region_map <- function(act) {
  a <- act[level == "spec_region" & !is.na(nbhds) & nbhds != ""]
  if (!nrow(a)) return(data.table(spec_k = character(), ssub_k = character(), key = character()))
  a[, spec_k := sub("\\|.*", "", key)]
  unique(a[, .(ssub_k = norm_key(unlist(strsplit(nbhds, ",", fixed = TRUE)))), by = .(key, spec_k)],
         by = c("spec_k", "ssub_k"))
}
...
build_matched <- function(path, trk, act = NULL, xp = NULL) {
  ...
  if (trk == "com" && !is.null(xp)) {
    # Specialty neighborhood (e.g. apartment nbhd 65) from EXTR_Parcel where the panel lacks it
    n_before <- sum(!is.na(s[["ssub_k"]]))
    s[, pid10 := pad0(pid, 10)]
    s[xp, on = .(pid10 = pid),
      ssub_k := fifelse(is.na(ssub_k) & spec_k %in% i.SpecArea, i.SpecSubArea, ssub_k)]
    s[, pid10 := NULL]
    message(sprintf("    spec_sub filled from EXTR_Parcel: %s -> %s rows",
                    format(n_before, big.mark = ","), format(sum(!is.na(s[["ssub_k"]])), big.mark = ",")))
  }
  ...
  message(sprintf("    matched %s of %s %s parcels | area=%s spec=%s spec_sub=%s src=%s",
                  format(nrow(m), big.mark = ","), format(n_fc, big.mark = ","), trk,
                  area_c, spec_c, ssub_c, src_c))
  if (!is.na(src_c)) {
...
make_cells <- function(m, trk, act = NULL) {
  out <- list()
  if (trk == "res") {
  ...
  if (trk == "com") {
    ...
    rmap <- if (is.null(act)) NULL else spec_region_map(act)
    if (!is.null(rmap) && nrow(rmap)) {
      mr <- merge(m[is_spec == TRUE & !is.na(ssub_k)], rmap, by = c("spec_k", "ssub_k"))
      s3 <- summ_cells(mr, "key")
      if (!is.null(s3)) {
        s3[, `:=`(kind = "specialty", level = "spec_region")]
        out[["spec_region"]] <- s3
      }
    }
...
run_area_backtest <- function() {
      ...
      for (trk in union(CFG_BT[["tracks_cells"]], CFG_BT[["tracks_context"]])) {
        ...
        if (trk %in% CFG_BT[["tracks_cells"]]) {
          ...
          for (md in names(subsets)) {
            cl <- make_cells(subsets[[md]], trk, act)
  ...
  cells[, cell_label := fifelse(track == "res", paste("Res", key),
                                fifelse(track == "condo", paste("Condo", fcoalesce(area_name, key)),
                                        fifelse(kind == "geo", paste("Com", key),
                                                fifelse(level == "spec_region",
                                                 paste0("Spec ", sub("\\|.*", "", key), " ", fcoalesce(area_name, key)),
                                                 paste("Spec", gsub("|", " / ", key, fixed = TRUE))))))]
  cells[, actual_scope := fcase(level == "spec_region", "specialty region (multi-city)",
                                kind == "specialty", "countywide specialty report",
                                default = "area report")]
  ...
  prim[, spec_root := fifelse(level %in% c("spec_sub", "spec_region"),
                              sub("\\|.*", "", key), NA_character_)]
  has_spec <- unique(prim[level == "spec", .(mode, track, spec_root = key, level = "spec_sub")])
  has_reg  <- unique(prim[level == "spec_region", .(mode, track, key = spec_root, level = "spec")])
  prim[, tiny := n_parcels < CFG_BT[["min_cell_parcels"]] | av_base < CFG_BT[["min_cell_av"]]]
  prim[, in_summary := !tiny]
  # Submarket rows would double count their specialty's headline row
  if (nrow(has_spec)) prim[has_spec, on = .(mode, track, spec_root, level), in_summary := FALSE]
  # Where regional rates exist, they replace the countywide headline (Seattle
  # sits in specific regions)
  if (nrow(has_reg)) prim[has_reg, on = .(mode, track, key, level), in_summary := FALSE]
  ...
  cap <- paste0("Matched-parcel growth TY", CFG_BT[["base_yr"]], " to TY", CFG_BT[["target_yr"]],
                ". Specialty actuals are countywide or regional; model cells are Seattle only.")
