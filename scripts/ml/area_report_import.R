...
#            (3) Commercial specialty reports (Major Office 280, etc.).
...
#   land_prev, land_curr, imps_prev, imps_curr   num  component AV (condo only)
#   pct_land, pct_imps    num  reported component % change as a decimal (condo only)
#   n_parcels      int    improved parcels in the population (condo only)
#   nbhds          chr    comma-separated condo neighborhoods covered (condo only)
#   area_name      chr    report area name, e.g. "Capitol Hill" (condo only)
  ...
  if (nrow(parsed) > 0) {
    ...
    area_report_actuals <- parsed |>
      ...
      dplyr::select(assessment_yr, prop_type, report_kind,
                    ...
                    dplyr::any_of(c("land_prev", "land_curr", "imps_prev", "imps_curr",
                                    "pct_land", "pct_imps", "n_parcels",
                                    "nbhds", "area_name")),
                    report_id, district, source_file) |>
      dplyr::arrange(prop_type, report_kind, area, spec_area, spec_sub, report_id, basis)
    ...
    dupes <- area_report_actuals |>
      dplyr::count(prop_type, report_kind, area, spec_area, spec_sub,
                   dplyr::across(dplyr::any_of("nbhds")), basis) |>
      dplyr::filter(n > 1)
    if (nrow(dupes) > 0)
